...
func TestVNCWebSocketProxy(t *testing.T) {
	...
	conn, resp, err := websocket.DefaultDialer.Dial(wsURL, http.Header{
		"Authorization": []string{"Bearer native-token"},
	})
	if err != nil {
	...
	auth := authSeen
	mu.Unlock()
	if auth != "Bearer native-token" {
		t.Fatalf("upstream should see client auth passed through, got %q", auth)
	}
	mu.Lock()
	host := hostSeen
	mu.Unlock()
	...
	if host != wantHost {
		t.Fatalf("upstream saw host %q, want client host %q", host, wantHost)
	}
}

func TestVNCWebSocketProxyTLS(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("dial proxied tls websocket: %v", err)
	}
	defer conn.Close()
	if resp.StatusCode != http.StatusSwitchingProtocols {
		t.Fatalf("expected 101, got %d", resp.StatusCode)
	...
	if err := conn.WriteMessage(websocket.TextMessage, []byte("ping")); err != nil {
		t.Fatalf("write through tls proxy: %v", err)
	}
	messageType, data, err := conn.ReadMessage()
	if err != nil {
		t.Fatalf("read through tls proxy: %v", err)
	}
	if messageType != websocket.TextMessage || string(data) != "ping" {
		t.Fatalf("unexpected tls echo: type=%d data=%q", messageType, data)
	}
}

func TestVNCProxyForwardsUnauthorized(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	request := fmt.Sprintf("GET /websockify HTTP/1.1\r\nHost: %s\r\nConnection: Upgrade\r\nUpgrade: websocket\r\nSec-WebSocket-Version: 13\r\nSec-WebSocket-Key: SGVsbG9XUw==\r\n\r\n", host)
	...
	if got := resp.Header.Get("WWW-Authenticate"); got != `Basic realm="Websockify"` {
